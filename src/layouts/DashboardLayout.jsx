    ...
    return (
        <div className="flex bg-slate-950 min-h-screen font-sans text-slate-50 selection:bg-blue-500/30">
            ...
            <aside className="w-20 lg:w-64 fixed h-screen z-40 bg-slate-950/50 backdrop-blur-xl border-r border-slate-800/50 flex flex-col transition-all duration-300">
                <div className="h-20 flex items-center justify-center lg:justify-start lg:px-6 border-b border-slate-800/50 relative overflow-hidden group">
                    ...
                    <LayoutGrid className="text-blue-500 animate-pulse-slow shrink-0" size={28} />
                    <span className="hidden lg:block ml-3 font-extrabold text-xl tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">
                        WorkSuite
                    </span>
                </div>
                ...
                <nav className="flex-1 py-8 px-3 space-y-2 overflow-y-auto custom-scrollbar">
                    ...
                    <NavLink to="/" className={({ isActive }) => `flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 group ${isActive ? 'bg-indigo-600 shadow-lg shadow-indigo-500/20 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                    ...
                    {(user?.role === ROLES.MANAGER || user?.role === ROLES.RESPONSIBLE) && (
                        <>
                            <NavLink to="/team" className={({ isActive }) => `flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 group ${isActive ? 'bg-blue-600 shadow-lg shadow-blue-500/20 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                            ...
                            <NavLink to="/reports" className={({ isActive }) => `flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 group ${isActive ? 'bg-cyan-600 shadow-lg shadow-cyan-500/20 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                ...
                <div className="p-4 border-t border-slate-800/50">
                    <div className="bg-slate-900/50 rounded-xl p-3 flex items-center gap-3 border border-slate-800 mb-3">
                        <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-md shrink-0">
                            {user?.avatar || 'U'}
                        </div>
