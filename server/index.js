...
app.get('/api/sync/productivity', async (req, res) => {
    ...
    try {
        const [sessions, records, groups, closed, incidents, families, logs] = await Promise.all([
            ...
            pool.query("SELECT id, employee_id as \"employeeId\", employee_name as \"employeeName\", start_time as \"startTime\", end_time as \"endTime\", duration_seconds as \"durationSeconds\", date, groups_count as \"groups\" FROM daily_records WHERE store_id = $1 AND start_time > NOW() - INTERVAL '15 days' ORDER BY start_time DESC", [storeId]),
            ...
            // REDUCED WINDOW: 7 days instead of 60 for sync egress optimization
            pool.query("SELECT * FROM transaction_logs WHERE store_id = $1 AND start_time > NOW() - INTERVAL '7 days' ORDER BY start_time DESC", [storeId])
        ]);
