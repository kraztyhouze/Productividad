        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
                ...
                <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-end border-b border-slate-50 pb-2">
                            <span className="text-[9px] font-black text-[#1A365D] uppercase">Total Flotante</span>
                            <span className="text-xl font-black text-rose-500">{(transitWeight + inOpsWeight).toFixed(2)} gr</span>
                        </div>
                        <div className="max-h-[60px] overflow-y-auto pr-2 space-y-1 mt-2">
                            {safeOrders.filter(o => o.status === 'Pedido Lanzado').map(o => (
                                <div key={o.id} className="flex justify-between items-center text-[8px] font-bold text-slate-400 uppercase">
                                    <span className="truncate max-w-[80px]">{o.partner_name}</span>
                                    <span className="text-[#1A365D]">{Number(o.est_weight).toFixed(1)}g</span>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {viewMode === 'ops' ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    {/* LEFT COLUMN: PRIMARY OPS (Inventory & Movements) */}
                    <div className="lg:col-span-8 space-y-6">
                        {/* A. Inventory Summary */}
                        <GoldsmithInventoryPanel inventory={safeInventory} onAdjust={onAdjustInventory} />
                        
                        {/* B. Movements Table */}
                        <div className="bg-white rounded-[40px] border border-[#E2E8F0] shadow-sm overflow-hidden flex flex-col">
                            <div className="p-8 pb-4 flex justify-between items-center bg-[#F8FAFC]/50">
                                <div>
                                    <h2 className="text-2xl font-black text-[#1A365D] tracking-tighter uppercase">Registro de Movimientos</h2>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Bitácora completa de entradas y salidas</p>
                                </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left uppercase text-[10px] font-bold min-w-[700px]">
                                    <thead className="bg-[#F8FAFC] font-black text-[#A0AEC0] tracking-widest border-b">
                                        <tr>
                                            <th className="p-6 pl-10">Fecha</th>
                                            <th className="p-6">Tipo</th>
                                            <th className="p-6">Agrupación</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100 bg-white">
                                        {safeMovements.slice(0, 15).map(m => (
                                            <tr key={m.id} className="hover:bg-slate-50/50 transition-colors group">
                                                <td className="p-6 pl-10 font-bold text-[#1A365D]">{format(parseISO(m.date), 'dd/MM/yyyy')}</td>
                                                <td className="p-6">
                                                ...
                                                <td className="p-6 font-black text-slate-600">{m.partner_name}</td>
                                                <td className="p-6 font-mono text-[#1A365D] font-black">{m.weight} gr</td>
                                                <td className="p-6">
                                                ...
                                                <td className="p-6 text-right pr-10">
                                                    <div className="flex items-center justify-end gap-3">
                                                        ...
                                                        <button onClick={() => onDeleteMovement(m.id)} className="text-slate-300 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition-all">
                                                            <Trash2 size={16}/>
                                                        </button>
                                                    </div>
                                                </td>
                    ...
                    {/* RIGHT COLUMN: SECONDARY OPS (Sidebar & Reminders) */}
                    <div className="lg:col-span-4 space-y-8">
                        {/* A. Quick Launch */}
                        <button onClick={() => onAddOrder()} className="w-full bg-[#1A365D] text-white py-6 rounded-[32px] font-black text-sm uppercase tracking-[0.2em] shadow-2xl shadow-blue-900/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-3 active:scale-95">
                            <PlusCircle size={20}/> LANZAR NUEVO PEDIDO
                        </button>

                        {/* B. Critical Alerts REMOVED AS REQUESTED */}

                        {/* C. Active Orders List */}
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black text-[#1A365D] uppercase tracking-widest mb-2 flex items-center gap-2">
                                <ShoppingCart size={14} className="text-blue-500"/> Pedidos en Gestión
                            </h4>
                        ...
                        <div className="bg-white p-8 rounded-[40px] border border-[#E2E8F0] shadow-sm">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xs font-black text-[#1A365D] uppercase tracking-widest flex items-center gap-2">
                                    Directorio Socios <Users size={16} className="text-[#FF8C9D]"/>
                                </h3>
                                <button onClick={onAddPartner} className="text-[9px] font-black text-[#FF8C9D] bg-rose-50 px-3 py-1.5 rounded-lg hover:bg-rose-100 transition-all uppercase">Añadir</button>
                            </div>
                            <div className="space-y-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                                {safePartners.map(p => (
                                    <div key={p.id} className="p-5 bg-slate-50/50 rounded-[32px] border border-transparent group transition-all hover:bg-white hover:border-slate-100 hover:shadow-lg">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <p className="text-[10px] font-black text-[#1A365D] uppercase tracking-tighter">{p.name}</p>
                                                <p className="text-[8px] font-bold text-slate-400 mt-0.5">{p.phone || 'Sin contacto'}</p>
                                            </div>
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all shadow-sm rounded-lg bg-white">
                                        ...
                                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-dashed border-slate-200">
                                            <span className="text-[8px] font-black text-slate-300 uppercase tracking-tighter">Deuda ({p.debt_type})</span>
                        ...
                        <div className="bg-slate-50/50 p-8 rounded-[40px] border border-slate-100">
                            <h4 className="text-[10px] font-black text-[#1A365D] uppercase tracking-widest mb-4 flex items-center gap-2">
                                <Clock size={14} className="text-blue-400"/> Historial Afinaje
                            </h4>
                                    ...
                                    smeltingHistory.map(m => (
                                        <div key={m.id} className="flex justify-between items-center p-3 bg-white rounded-2xl border border-slate-50">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-[8px] font-bold text-slate-400">Rendimiento</p>
                                            </div>
                                        </div>
                                    ))
                                )}
        ...
        <div className="space-y-8 animate-in zoom-in duration-300">
            <div className="bg-white p-8 rounded-[40px] border border-[#E2E8F0] shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                    <button 
                        onClick={() => downloadJewelryPDF(movements)}
                        className="px-6 py-3 bg-[#1A365D] text-white rounded-2xl flex items-center gap-3 text-[10px] font-black uppercase hover:shadow-xl transition-all shadow-sm"
                    >
                        <FileText size={16} className="text-[#FF8C9D]"/> Exportar PDF
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="bg-white p-6 rounded-[32px] border border-[#E2E8F0] shadow-sm">
                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest block mb-1">Coste Invertido</span>
                    <p className="text-3xl font-black text-[#1A365D] tracking-tighter">{totalStats.totalCost.toFixed(2)}€</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-[#E2E8F0] shadow-sm">
                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest block mb-1">Beneficio Neto</span>
                ...
                <div className="bg-green-50 p-6 rounded-[32px] border border-green-100 shadow-sm">
                    <span className="text-[8px] font-black text-green-600/60 uppercase tracking-widest block mb-1">Rentabilidad (%)</span>
